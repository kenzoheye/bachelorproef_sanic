...
async def allowed_route(request):
    ...
    if request.json is None:
        return json(
            {"domain": "/allowed", "detail": "No body provided", "code": 400},
            status=400,
        )
    ...
    if URI is None:
        return json(
            {"domain": "/allowed", "detail": "No URI provided", "code": 400}, status=400
        )
    if method is None:
        return json(
            {"domain": "/allowed", "detail": "No method provided", "code": 400},
            status=400,
        )
    if ip is None:
        return json(
            {"domain": "/allowed", "detail": "No IP provided", "code": 400}, status=400
        )
    else:
    ...
    try:
        ...
        uri = host[host.find("/") : len(host)]  # NOQA
        hosts = {
